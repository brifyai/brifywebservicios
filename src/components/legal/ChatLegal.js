...
const ChatLegal = () => {
  ...
  const searchLaws = async (query) => {
    try {
      ...
      const embeddingResult = await embeddingsService.generateEmbedding(query, user?.id || user?.email)
...
${fileContent.substring(0, 2000)}`
        ...
        const analysisText = await minimaxService.createMessage({
          system: '',
        ...
        documentAnalysis = analysisText || 'No se pudo generar análisis'
